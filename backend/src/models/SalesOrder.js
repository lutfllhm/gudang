...
class SalesOrder {
  ...
  static async updateStatus(id, status) {
    ...
    // Validasi status sesuai label baku Accurate
    const validStatuses = ['Terproses', 'Sebagian diproses', 'Menunggu diproses'];
  ...
  static async getStats({ startDate = null, endDate = null } = {}) {
    ...
    const stats = await query(`
      SELECT 
        ...
        SUM(CASE WHEN status = 'Menunggu diproses' THEN 1 ELSE 0 END) as pending,
        SUM(CASE WHEN status = 'Sebagian diproses' THEN 1 ELSE 0 END) as partial,
        SUM(CASE WHEN status = 'Terproses' THEN 1 ELSE 0 END) as completed,
  ...
  static async getPendingOrders(limit = 20) {
    const orders = await query(
      `SELECT * FROM sales_orders 
       WHERE is_active = 1 AND status = 'Menunggu diproses'
