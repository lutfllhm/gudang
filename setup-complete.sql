-- =============================================
-- iWare Warehouse - Complete Database Setup
-- Copy file ini ke VPS dan jalankan dengan:
-- mysql -u root -p < setup-complete.sql
-- =============================================
...
CREATE USER 'iware'@'%' IDENTIFIED BY 'iware_password';
CREATE USER 'iware'@'172.17.0.1' IDENTIFIED BY 'iware_password';
...
CREATE TABLE IF NOT EXISTS sales_orders (
  ...
  keterangan TEXT,
  status VARCHAR(100) DEFAULT 'Menunggu Diproses',
...
SELECT 
  id, item_id, nama_item, kode_item, kategori, satuan,
  stok_tersedia, harga_jual, harga_beli, last_sync
FROM items
WHERE is_active = TRUE;

CREATE OR REPLACE VIEW v_active_sales_orders AS
SELECT 
  id, so_id, nomor_so, tanggal_so, nama_pelanggan,
  status, total_amount, last_sync
FROM sales_orders
WHERE is_active = TRUE
...
SELECT 
  ...
  (SELECT COUNT(*) FROM sales_orders WHERE is_active = TRUE AND status = 'Menunggu Diproses') as pending_orders,
...
SELECT '========================================' as '';
SELECT 'Email: superadmin@iware.id' as '';
SELECT 'Password: admin123' as '';
SELECT 'IMPORTANT: Change password after first login!' as '';
SELECT '========================================' as '';
